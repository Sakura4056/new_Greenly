...
public class ReminderController {
    ...
    @Autowired
    public ReminderController(ReminderService reminderService, JwtUtil jwtUtil, com.plant.backend.task.CareTask careTask) {
    ...
    public Result<ReminderConfig> getConfig(HttpServletRequest httpRequest) {
        String token = httpRequest.getHeader("Authorization").substring(7);
        Long userId = jwtUtil.getUserIdFromToken(token);
        return Result.success(reminderService.getConfig(userId));
    }
